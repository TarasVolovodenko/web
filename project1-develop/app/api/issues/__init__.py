...
from models import Issue, Comment,Photo, IssueState, PollutionCategory, Role, User
...
class IssueWrapper(metaclass=Singleton):
    ...
    def fixed_issues_info(self, fields):
        session = self.session_maker()
        ...
        fixed_issues = session.query(Issue).filter(Issue.state.state == 'fixed')

        return process(fixed_issues)
